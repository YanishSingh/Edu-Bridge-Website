...
import { partnerApi, setPartnerToken } from '../utils/api';
...
export default function PartnerLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const data = await partnerApi.login({
      ...
      if (err.rejectionReason) setError((prev) => prev + ' ' + err.rejectionReason);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 flex items-center justify-center px-3 sm:px-4 py-6 sm:py-8 md:py-12">
      ...
      >
        ...
        >
          ...
          <p className="text-gray-600 text-xs sm:text-sm md:text-base px-2">Log in to access your partner vault</p>
        </motion.div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  maxLength={255}
                  required
                  className="w-full pl-9 sm:pl-11 pr-3 sm:pr-4 py-2.5 sm:py-3 rounded-lg sm:rounded-xl border border-gray-200 bg-white text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all duration-200"
                />
              </div>
            </div>
        ...
        <motion.p
          ...
          className="text-center text-gray-600 text-xs sm:text-sm mt-6 sm:mt-8 px-2"
        >
          Don&apos;t have an account?{' '}
          <Link to="/signup" className="text-primary font-semibold hover:text-primary-dark transition-colors inline-flex items-center gap-1">
            Sign up
